...
- **Base packages** — CLI utilities (bat, zsh, fzf, starship, gh, git, …) and Python system packages (python3-venv) — runs on headless servers too
- **Desktop packages** — GNOME GUI tools (gnome-tweaks, gnome-shell-extensions)
- **Editors** — VS Code (Microsoft APT repo) and Neovim (snap); GitHub CLI installs from the Ubuntu archive via `setup_base`
...
- **`setup_base`** — CLI utilities (bat, zsh, fzf, starship, gh, git, …) and Python system packages (python3-venv) — runs on headless servers
- **`setup_desktop`** — GNOME GUI tools (gnome-tweaks, gnome-shell-extensions)
- **`setup_editors`** — VS Code (Microsoft APT repo) and Neovim (snap); GitHub CLI installs from the Ubuntu archive via `setup_base`
