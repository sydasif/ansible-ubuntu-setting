...
- **Containerlab installs from GitHub releases** (`roles/setup_containerlab` downloads the `.deb` directly from `github.com/srl-labs/containerlab/releases`). Fury APT (netdevops.fury.site) is skipped due to intermittent 404s. Upstream publishes amd64/arm64 assets only, so the role asserts the architecture up front. Bump `containerlab_version` in `roles/setup_containerlab/vars/main.yml` to upgrade — the install block is gated on `dpkg-query` not already reporting that version, so re-runs skip the download, verify the release `checksums.txt` when they do run, and remove the `/tmp` artifacts afterwards.
