...
- **Containerlab installs from GitHub releases** (`roles/setup_containerlab` downloads the `.deb` directly from `github.com/srl-labs/containerlab/releases`). Fury APT (netdevops.fury.site) is skipped due to intermittent 404s. Bump `containerlab_version` in `roles/setup_containerlab/vars/main.yml` to upgrade. The `dpkg -i` task always reports `changed` because `changed_when: _clab_install.rc == 0` is always true when the deb is already installed — this is intentional, not an idempotency bug to fix.
